...
test.describe('GM: handleSheetClick dispatch (document API)', () => {
    ...
    test('issue #363: equipping armor via item-toggle applies its Active Effect stat bonus, unequipping removes it', async ({
        page
    }) => {
        const result = await page.evaluate(async () => {
            const stamp = Date.now();
            const [actor] = await Actor.createDocuments([
                {
                    name: `E2E Armor Effect ${stamp}`,
                    type: 'character',
                    system: { stats: { str: { value: 3, bonus: 0 } } }
                }
            ]);
            const [armor] = await actor.createEmbeddedDocuments('Item', [
                { name: `E2E Effect Armor ${stamp}`, type: 'armor', system: { equipped: false } }
            ]);
            ...
            const equippedEffectCount = equippedActor.effects.filter((e) => e.origin === armor.uuid).length;
            ...
            const unequippedEffectCount = unequippedActor.effects.filter((e) => e.origin === armor.uuid).length;
