...
- **Vehicle sheet max-HP edit/persist coverage:** `regression-vehicle-sheet.spec.js` previously only asserted that `system.hp.max` _displayed_ the actor's initial value, mirroring the same edit-persistence gap the NPC sheet had before its 2.8.7 fix. A new test now edits the field and polls the actor document to confirm the change actually persists (it does — vehicles already share the NPC/Threat "GM-authored, not overwritten by derived data" HP-max behavior via `resolveDerivedHpMax`). No behavior change; closes the coverage gap.
- **Settings-menu toggle/persist coverage:** every other settings test read/wrote `game.settings` directly; a new test in `regression-sla.spec.js` flips a real checkbox in the Configure Settings UI and confirms it round-trips through `game.settings`, restoring the original value afterward.
- **Dedicated accessibility/keyboard-only spec:** new `tests/e2e/regression-accessibility.spec.js` drives sheet tab rails and a dialog by keyboard alone (Arrow/Home/End tab navigation, Escape-to-close), covering the two behaviors added below.
- **Tab rail keyboard navigation (Arrow/Home/End):** Both sheet tab rails (`nav.sheet-tabs[role="tablist"]`) already carried the correct WAI-ARIA `role="tab"`/roving-`tabindex` markup, but nothing implemented the WAI-ARIA APG "Tabs" keyboard pattern the markup promises — Tab reached the active tab, but Arrow/Home/End did nothing. New shared helper `module/helpers/tab-keyboard-nav.mjs` (`resolveTabKeyNavIndex` is pure and unit-tested; `bindTabKeyboardNav` wires it to the DOM) is now used by both `SlaActorSheet` and `SlaItemSheet`: ArrowRight/ArrowDown and ArrowLeft/ArrowUp move focus to and activate the next/previous tab (wrapping at the ends), Home/End jump to the first/last tab.
- **Escape-to-close on all five dialogs:** `XPDialog`, `LuckDialog`, and `SlaSimpleContentDialog` (which also hosts the Attack and Reload dialog content) all extend plain `ApplicationV2` rather than `Dialog`/`DialogV2`, so none of them had Foundry's usual Escape-to-close behavior — Escape did nothing on any of the system's own dialogs. New shared helper `module/helpers/dialog-keyboard.mjs` (`bindEscapeToClose`) is now wired into all three classes' `_onRender`.
- **E2E viewport raised to Foundry's own documented minimum (1366×768):** `playwright.config.js`'s top-level `use.viewport` said `1920×1080`, but the `chromium` project's `use: { ...devices['Desktop Chrome'] }` spread in that device preset's own `viewport: 1280×720`, which silently won the merge — every E2E test had actually always run below Foundry's minimum supported resolution, triggering its persistent "screen resolution too small" warning toast on every run (see `.docs/LESSONS_LEARNED.md`'s viewport entry). Both the top-level default and the `chromium` project now explicitly set `1366×768`; verified live that the warning is gone and re-ran 23 UI-heavy tests (actor sheets, dialogs, the new accessibility spec) back-to-back with zero failures. Left at 1366×768 rather than the originally-documented 1920×1080 — that larger jump was already found (same lessons-learned entry) to destabilize this sandbox's software-rendered Chromium.

### Fixed

- **New weapon and Ebb Formula items no longer ship with misleading pre-filled links:** `SlaWeaponData.skill` and `SlaEbbFormulaData.discipline` (`module/data/item.mjs`) declared `initial: 'pistol'`/`initial: 'blast'`, so every newly created weapon — melee included — and every new Ebb Formula started already linked to the wrong skill/discipline, contradicting the empty drag-and-drop zone `.docs/item_setup.md` documents. Both fields now start empty, matching the existing empty-by-default pattern already used by `SlaMagazineData.linkedWeapon` and `SlaEbbFormulaData.skill` in the same file.
- **Notes tab now shows an empty-state hint:** every other empty section on a sheet (Species/Package drop zones, Active Effects, Inventory) shows explanatory placeholder copy; the Notes `<prose-mirror>` editor rendered as a bare black box with no indication it was editable. Added a localized hint (`SLA.ActorSheet.NotesEmpty`) shown via `{{#unless system.notes}}` above the editor on the character, NPC, vehicle, and legacy bio-traits Notes panels, reusing the existing `.sla-empty-state` styling.
- **Removed a deprecated Foundry API read that fired on every actor render:** `resolveActiveEffectAddMatcher()` (`module/documents/derived/active-effects.mjs`) read `CONST.ACTIVE_EFFECT_MODES` via its default parameter to resolve the legacy pre-v14 numeric ADD mode. Foundry v14 wraps that property in a deprecation-warning getter that logs on every _read_, not just writes, so this fired on every derived-data pass for any actor with active effects. The function already hardcoded the legacy value (`2`) as a fallback, so the property read was unnecessary; it's now hardcoded outright with no behavior change.
- **Manually clearing Stunned no longer gets silently re-applied by an unrelated wound edit:** `SlaActor._calculateWounds()` (`module/documents/actor.mjs`) force-derived the displayed Stunned condition from the head wound on every render, overriding a GM manually clearing the Stunned status icon (representing rest/drugs/medical intervention, per the rulebook) as long as the head wound remained marked. Separately, `_handleWoundEffects()` re-evaluated the head-wound-driven Stunned Active Effect on _any_ wound field changing, not just the head field, so editing an unrelated wound (e.g. a leg) could resurrect a Stunned effect that had just been manually removed. Stunned now only re-derives from the head wound when the head field itself changes (`resolveStunnedFromHeadWound()` in `module/documents/derived/wounds.mjs`), and the displayed condition always mirrors the real Active Effect state.
- **Two E2E regression tests fixed after drifting from earlier changes in this release (CI-only, no production behavior change):** `regression-vehicle-sheet.spec.js`'s weapon-drop-zone test used an unscoped `.sla-empty-state` locator that matched two elements once the Notes empty-state hint (above) was added to the same sheet, causing a strict-mode violation; the locator is now scoped to the weapons section specifically. `regression-sla.spec.js`'s settings-checkbox-persist test shared its 60s budget with a 4-click Settings-navigation chain that alone takes ~50s in CI, leaving no headroom for the checkbox/Save/poll steps on top; its timeout is now 120s.
...
- **Ammo modifiers never engaged through Reload:** Reloading a weapon never recorded what ammo type was loaded, so every ammo modifier fix above (and the display below) was unreachable in normal play regardless of what magazine was used — every shot behaved as standard ammo. Reloading now snapshots the magazine's ammo type onto the weapon, which the combat rolls read directly. **Note:** this snapshot only happens at Reload time — weapons already loaded before this update won't show any ammo modifier until reloaded again (even with the same magazine); this is expected, not a new bug. The GM will see a one-time in-app reminder after updating.
