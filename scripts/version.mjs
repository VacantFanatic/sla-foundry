...
const unreleasedContent = (nextSectionMatch ? afterHeader.slice(0, nextSectionMatch.index) : afterHeader).trim();
...
let newChangelog = changelog.replace(UNRELEASED_HEADER + '\n', `${UNRELEASED_HEADER}\n\n## [${version}] - ${today}\n`);
...
if (unreleasedLinkRegex.test(newChangelog)) {
    newChangelog = newChangelog.replace(unreleasedLinkRegex, `${newUnreleasedLink}\n${newVersionLink}`);
} else {
