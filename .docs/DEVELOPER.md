...
Core stats (`str`, `dex`, `know`, `conc`, `cha`, `cool`) each have a `value` (sheet base) and a `bonus` field. Active Effects should target `system.stats.<key>.bonus` to apply temporary modifiers without overwriting the base. `system.rollModifier.bonus` is the same pattern applied to a single standing modifier for _every_ roll type (skill, stat check, weapon, explosive, Ebb), summed into `system.rollModifier.total` the same way — unlike the core stats it has no player-editable `value`, so Active Effects are the only way to set it.
